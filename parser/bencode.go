...
// ParseBencode takes a bencode string as input and returns a triple:
// - first value: the remaining string that is not parsed yet. If there is an error that occurs, the first value is the original input value.
// - second value: parsed value, the type is one of the following: 64-bit integer, list, dictionary, string.
...
func ParseBencode(data string) (string, any, error) {
	if data[0] == 'i' {
		...
		return remaining[1:], dict, nil // consume 'e'
	} else {
